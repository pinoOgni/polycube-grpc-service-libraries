...
int main(int argc, char **argv) {
  ...
  // salvare helloworld2 in una var dentro polycubeclient
  
  polycubeclientgrpc.Subscribe("helloworld2",uuid);
  std::cout << "After Subscribe " << std::endl;

  // register paths
  polycubeclientgrpc.RegisterHandlerDel(":cubeName/",&destroyCube);
  polycubeclientgrpc.RegisterHandlerPost(":cubeName/",&createCube);

  // pino: TODO a little bit strange this first solution
  polycubeclientgrpc.RegisterHandlerOptions("helloworld2",&helpCommands); 
  ...
  polycubeclientgrpc.RegisterHandlerDel(":cubeName/ports/:portName/",&delPort);
...
ToPolycubed getAction(ToServiced request, PolycubeClient* polycubeclientgrpc) {
  ...
  //std::cout << "GETACTION ACTION " << static_cast<std::underlying_type<Helloworld2ActionEnum>::type>(tableGetReply.bytevalues().operator[0]) << std::endl;
...
ToPolycubed setAction(ToServiced request, PolycubeClient* polycubeclientgrpc) {
  ...
  // Helloworld2ActionEnum value = Helloworld2JsonObject::string_to_Helloworld2ActionEnum(request_body);
  // uint8_t action = static_cast<uint8_t>(Helloworld2ActionEnum::DROP);
  uint8_t action = 1;
...
ToPolycubed createCube(ToServiced request, PolycubeClient* polycubeclientgrpc) {
    ...
    ToPolycubed ret_reply;
    auto create_cube_reply = polycubeclientgrpc->CreateCube(request.serviced_info().cube_name(),helloworld2_code_ingress,helloworld2_code_egress,conf);
