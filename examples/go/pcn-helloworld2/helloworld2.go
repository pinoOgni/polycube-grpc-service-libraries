...
func main() {
	...
	// pb.RegisterHandlerPatch(":cubeName/ports/:portName/peer/",&setPeer);
...
func GetAction(request pb.ToServiced) pb.ToPolycubed {
	...
	tableGetReply := pb.TableGet(request.GetServicedInfo().GetCubeName(), "action_map", pb.CubeInfo_INGRESS, key, unsafe.Sizeof(key), unsafe.Sizeof(value))
		if (tableGetReply == nil) {
			return SetToPolycubedReply(false,`{"message":"error while getting the action"}`)		
		} else {
			...
			return SetToPolycubedReply(true,`{"action":"`+action+`"}`)
		}		
}
...
func SetAction(request pb.ToServiced) pb.ToPolycubed {
	...
	var key uint32 = 0
	// var value uint8 = 1
	tableSetReply := pb.TableSet("h1","action_map",pb.CubeInfo_INGRESS,key,unsafe.Sizeof(key),action,unsafe.Sizeof(action))
	fmt.Println("request.GetServicedInfo().GetCubeName() ", request.GetServicedInfo().GetCubeName())
	fmt.Println("tableSetReply: ", tableSetReply)
	if tableSetReply.GetStatus() == true {
		return SetToPolycubedReply(true,"ACTION SETTED")
	} else {
		return SetToPolycubedReply(false,"ACTION NOT SETTED")
	}
}
...
func CreateCube(request pb.ToServiced) pb.ToPolycubed {
	...
	if _, ok := cubes[cube_name]; ok {
		return SetToPolycubedReply(false,cube_name + " already created")
	}
	...
	if create_cube_reply.GetStatus() == true {
		...
		cubes[cube_name] = make([]string, 0, 2)
		return SetToPolycubedReply(true,"CREATED")
	} else {
		return SetToPolycubedReply(false,"NOT CREATED")
	}
...
func DestroyCube(request pb.ToServiced) pb.ToPolycubed {
	...
	if destroy_cube_reply.GetStatus() == true {
		...
		return SetToPolycubedReply(true,"DESTROYED")
	} else {
		return SetToPolycubedReply(false,"NOT DESTROYED")
	}
}
...
func UpdatePortsMap(request pb.ToServiced) pb.ToPolycubed {
	...
	if ok == true {
		return SetToPolycubedReply(true,"OK")
	} else {
		return SetToPolycubedReply(false,"KO")
	}
}
...
func AddPorts(request pb.ToServiced) pb.ToPolycubed {
	...
	if len(cubes[cube_name]) >= 2 {
		...
		return SetToPolycubedReply(false,`{"message":"Maximum number of ports reached"}`)
	}
	...
	if setPortReply.GetStatus() == true {
		...
		return SetToPolycubedReply(false,`{"message":"Problem while adding port"}`)
	}
}
...
func DelPort(request pb.ToServiced) pb.ToPolycubed {
	...
	if len(cubes[cube_name]) == 0 {
		...
		return SetToPolycubedReply(false,`{"message":"There are no port to be deleted"}`)
	}
	...
	if pos == cap(cubes[cube_name]) {
		...
		// pino: TODO the message is not displayed by the terminal of polycubectl
		return SetToPolycubedReply(false,`{"message":"The port does not exists!"}`)
	}
...
func SetToPolycubedReply(success bool, message string) pb.ToPolycubed {
...
func ShowPorts(request pb.ToServiced) pb.ToPolycubed {
	...
	if len(cubes[cube_name]) != 0 {
		...
		return SetToPolycubedReply(true,`{"ports":"`+message+`"}`)
	} else {
		message := "There are no ports on the cube " + cube_name 
		return SetToPolycubedReply(false,`{"ports":"`+message+`"}`)
	}
}
...
func HelpCommands(request pb.ToServiced) pb.ToPolycubed {
	...
	return SetToPolycubedReply(true,message)
}